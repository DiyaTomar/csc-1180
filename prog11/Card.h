...
class Card {
    private:
        ...
        Suit suit;
        int value;
        void setValue();//to update the value after setting the card face
        //this method is private to prevent external entities from calling it
        //method is safe though, as it uses the proper code to set the value
    public:
        Face getFace() const;
        Suit getSuit() const;
        void setFace(Face);
...
//no validation is really needed, as the compiler will catch invalid values
    face = f;
    setValue();//update the value after changing the face value
}
